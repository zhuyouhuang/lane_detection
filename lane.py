...
def warper(img, M):
    #http://www.cnblogs.com/pakfahome/p/3914318.html
...
def create_output_frame(offcenter, pts, undist_ori, fps, curvature, curve_direction, binary_sub, threshold=0.6):
    ...
    return output,newwarp,color_warp
...
def process_frame(img, visualization=False):
    ...
    ploty = np.linspace(0, binary_warped.shape[0]-1, binary_warped.shape[0])#A到B，C个
    ...
    offcenter, pts,pts_left,pts_right = compute_car_offcenter(ploty, left_lane.mean_fitx, right_lane.mean_fitx, img_undist)
    ...
    #0
    output ,newwarp,color_warp= create_output_frame(offcenter, pts, img_undist_, fps, curvature, curve_direction, binary_sub)
    return output,img_undist_,newwarp,pts,pts_left,pts_right,color_warp
